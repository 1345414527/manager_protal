        ...
        <el-card class="box-card">
            ...
            <el-tabs tab-position="left" :before-leave="confirmInfo">
                ...
                <el-tab-pane label="题目选择" clas="tabPane">
                    ...
                    <el-row justify="first" type="flex" v-if="testInfo.select.length>0" v-for="(topic,index) in select" :key="topic.id">
                        <el-col :offset="1">
                            ...
                            <li :class="['select_item',{'correct_answer_select': topic.answer===(index+'')}]" v-for="(value,key,index) in topic.select" :key="key">{{key|filterSelect}}:  {{value}}</li>
                        </el-col>
                    </el-row>
                    ...
                    <el-row justify="first" type="flex" v-if="testInfo.judge.length>0" v-for="(topic,index) in judge" :key="topic.id">
                        <el-col :offset="1">
...
<script>
    ...
    import {insertTest,updatePublish} from "network/test/testHttp";
    ...
    export default {
        ...
        created() {
            ...
            this.testInfo.creator=this.$store.state.user.username
        },
        methods: {
            //查询所有的学科信息
            querySubjects(){
            ...
            confirmInfo(activeName, oldActiveName){
                ...
                if(oldActiveName==='0') {
                    ...
                    if(this.selectTopics.length===0) {
                        ...
                        insertTest(this.testInfo).then((res)=>{
                            if(res.result_code===251){
                                ...
                                this.$notify({title: '错误',message: '试卷创建失败',type: 'error'})
                                flag=false
                            }
                        })
                }
