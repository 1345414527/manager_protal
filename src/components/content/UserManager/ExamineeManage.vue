<template>
    <div class="examinee-manage">
        ...
        <el-table :data="publicTests" :stripe="true"  border max-height="550"   style="width: 100%" @sort-change="sortChange" :sort-orders="['ascending', 'descending']"
                  cell-class-name="test-cell" header-cell-class-name="test-header"  @cell-dblclick="celldblclick">
            <el-table-column type="index"></el-table-column>
            <el-table-column prop="id" label="id" width="230" sortable="custom"></el-table-column>
            <el-table-column prop="uname" label="用户名称" :width="150"></el-table-column>
            ...
            <el-table-column  label="分数" :width="120"  fixed="right" sortable="custom" prop="score">
                <template slot-scope="scope">
                    <el-tooltip class="item" effect="dark" :enterable="false" content="学生的考试分数" placement="top-start">
                        <el-tag type="danger" class="exam-status" v-if="scope.row.score<60&&scope.row.status===2"  >{{scope.row.score|scoreFilter}}分</el-tag>
                        <el-tag type="success" class="exam-status" v-else  >{{scope.row.score|scoreFilter}}分</el-tag>
                    </el-tooltip>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" min-width="210"  fixed="right" class-name="operateButton">
                <template slot-scope="scope">
                    ...
                    <el-tooltip class="item" effect="dark" :enterable="false" content="再次考试(不会重置学生的成绩)" placement="top-start">
                        <el-button  @click="againExam(scope.row)" type="success" icon="el-icon-plus" circle class="edit"></el-button>
                    </el-tooltip>
                    <el-tooltip class="item2" effect="dark" :enterable="false" content="重置考试" placement="top-start">
                        <el-button @click="resetExam(scope.row)" type="primary" icon="el-icon-refresh" circle></el-button>
                    </el-tooltip>
                    <el-tooltip class="item2" effect="dark" :enterable="false" content="删除考试" placement="top-start">
                        <el-button @click="deleteExam(scope.row)" type="danger" icon="el-icon-delete" circle></el-button>
                    </el-tooltip>
                </template>
            </el-table-column>
        </el-table>
...
<script>
    import {queryAllSubscribeExamInfo,deleteSubscribeByManager} from "network/exam/subscribeExamHttp";
    import {queryAllSubjects} from "network/subjects/subjectHttp";
    import utils from "utils/utils";
    import {again,reset} from "network/userinfo/examieeHttp";
    ...
    export default {
        ...
        methods: {
            ...
            //重置考试(分数保留)
            againExam(row){
                again(row.id).then(res=>{
                    if(res.result_code===255){
                        row.status=res.status
                        this.$notify({title: '成功', message: '考试重置成功！', type: 'success', offset: 100, duration: 2000})
                    }else{
                        this.$notify({title: '失败', message: '考试重置失败(只有为已考试状态才能重置)！', type: 'error', offset: 100, duration: 2000})
                    }
                })
            },
            //重置考试
            resetExam(row){
                reset(row.id).then(res=>{
                    if(res.result_code===255){
                        ...
                        this.$notify({title: '失败', message: '考试重置失败！', type: 'error', offset: 100, duration: 2000})
                    }
                })
            },
            //预览试卷
            ...
            //删除考试
            deleteExam(row){
                deleteSubscribeByManager(row.id).then(res=>{
                    if(res.result_code===251){
                        this.$notify({title: '成功', message: '考试订阅取消成功！', type: 'success', offset: 100, duration: 2000})
                        //删除信息
                        this.publicTests.splice(this.publicTests.findIndex(tests=>tests.id===row.id),1)
                    }else{
                        this.$notify({title: '失败', message: '考试订阅取消失败！！', type: 'error', offset: 100, duration: 2000})
                    }
                })
            },
            closePreviewDrawer(){
